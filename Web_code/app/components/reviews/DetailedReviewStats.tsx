...
const DetailedReviewStats: React.FC<DetailedReviewStatsProps> = ({
  reviews,
}) => {
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
        ...
        <div className="space-y-2">
          {[5, 4, 3, 2, 1].map((rating) => (
            <div
              key={rating}
              className="flex items-center gap-3 group hover:bg-gray-50 p-1 rounded transition-colors"
            >
              ...
              <div className="flex-1 bg-gray-200 rounded-full h-2 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${
                      maxRating > 0
                        ? (ratingDistribution[rating - 1] / maxRating) * 100
                        : 0
                    }%`,
                  }}
                />
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {categories.map(({ key, value, icon: Icon, label, color }) => (
            <div
              key={key}
              className="group relative bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-3 hover:from-white hover:to-gray-50 border border-gray-200 hover:border-gray-300 transition-all duration-200 hover:shadow-sm"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div
                    className={`w-6 h-6 ${color} rounded-md flex items-center justify-center group-hover:scale-110 transition-transform duration-200`}
                  >
                ...
                <div className="text-right">
                  ...
                  {value > 0 && (
                    <div className="text-xs text-gray-500">{t("from")} 5</div>
                  )}
                </div>
              </div>
            </div>
